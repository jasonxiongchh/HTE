...
train_save_path = r"F:\uqstudy\project\code\electricity_usage_monitoring\test\HTE_data\Electricity_HTE_trainData_normalized_reshaped_20_MORE.csv"
test_save_path = r"F:\uqstudy\project\code\electricity_usage_monitoring\test\HTE_data\Electricity_HTE_testData_normalized_reshaped_20_MORE.csv"
test_day_save_path = r"F:\uqstudy\project\code\electricity_usage_monitoring\test\HTE_data\Electricity_HTE_test_day_Data_normalized_reshaped_20_MORE.csv"
test_month_save_path = r"F:\uqstudy\project\code\electricity_usage_monitoring\test\HTE_data\Electricity_HTE_test_month_Data_normalized_reshaped_20_MORE.csv"
...
for data in [data_normalized_train, data_normalized_test, 
             data_normalized_test_oneday_modification, 
             data_normalized_test_onemonth_modification]:
...
if isDataChanged:
    ...
    test_data_for_model_oneday_modification = reshape_data(data_normalized_test_oneday_modification[final_features].values, time_steps, len(final_features))
    ...
    save_reshaped_data(test_data_for_model_oneday_modification, final_features, time_steps, test_day_save_path)
    save_reshaped_data(test_data_for_model_onemonth_modification, final_features, time_steps, test_month_save_path)
    ...
    test_data_for_model_oneday_modification = load_reshaped_data(test_day_save_path, final_features, time_steps)
    test_data_for_model_onemonth_modification = load_reshaped_data(test_month_save_path, final_features, time_steps)
...
# define model
def build_cnn_lstm_model(time_steps, features_count):
...
# training
if not bload_model:
    cnn_lstm = build_cnn_lstm_model(time_steps,len(final_features))
    cnn_lstm.summary()
    ...
    cnn_lstm.save('cnn_lstm_IF.keras')
else:
    cnn_lstm = load_model('cnn_lstm_IF.keras', compile=False)
...
if test_importance:
    ...
    output_filename_a = 'feature_importance_cnn_lstm_IF_permutation.csv'
...
# ====================
normalized_test_subset = data_normalized_test_onemonth_modification[columns_to_normalize].iloc[time_steps - 1:time_steps - 1 + len(result)].copy()
...
# === Step: Predict on training data to get training errors for IF ===
if not IF_is_well_trained:
    print("Calculating training residuals for IF...")

    train_predictions = cnn_lstm.predict(train_data_for_model)
    train_predictions = train_predictions.flatten()
    y_train_true = y_train.flatten()

    train_errors_mse = (y_train_true - train_predictions) ** 2
    train_errors_mae = np.abs(y_train_true - train_predictions)

    # Fit Isolation Forest models
    iso_mse = IsolationForest(**iso_forest_params_mse)
    iso_mae = IsolationForest(**iso_forest_params_mae)

    iso_mse.fit(train_errors_mse.reshape(-1, 1))
    iso_mae.fit(train_errors_mae.reshape(-1, 1))

    # Save models if needed
    with open("IF_model_mse.pkl", "wb") as f:
        pickle.dump(iso_mse, f)
    with open("IF_model_mae.pkl", "wb") as f:
        pickle.dump(iso_mae, f)

else:
    with open("IF_model_mse.pkl", "rb") as f:
        iso_mse = pickle.load(f)
    with open("IF_model_mae.pkl", "rb") as f:
        iso_mae = pickle.load(f)

# === Step: Predict on one-month modified test set ===
print("Predicting on one-month modified test set...")
predicted_modified_month = cnn_lstm.predict(test_data_for_model_onemonth_modification)
predicted_modified_month = predicted_modified_month.flatten()
true_modified_month = data_normalized_test_onemonth_modification['watt'].values[time_steps - 1:time_steps - 1 + len(predicted_modified_month)]

# === Step: Compute residuals ===
test_errors_mse = (true_modified_month - predicted_modified_month) ** 2
test_errors_mae = np.abs(true_modified_month - predicted_modified_month)

# === Step: Use IF to detect anomalies based on residuals ===
anomaly_scores_mse = iso_mse.decision_function(test_errors_mse.reshape(-1, 1))
anomaly_labels_mse = iso_mse.predict(test_errors_mse.reshape(-1, 1))

anomaly_scores_mae = iso_mae.decision_function(test_errors_mae.reshape(-1, 1))
anomaly_labels_mae = iso_mae.predict(test_errors_mae.reshape(-1, 1))

# Convert IF outputs to binary anomaly labels: -1 -> 1 (anomaly), 1 -> 0 (normal)
anomaly_labels_mse = (anomaly_labels_mse == -1).astype(int)
anomaly_labels_mae = (anomaly_labels_mae == -1).astype(int)

# === Step: Combine MAE and MSE anomaly labels ===
final_anomaly_labels = ((anomaly_labels_mse == 1) & (anomaly_labels_mae == 1)).astype(int)

# === Step: Evaluate performance ===
true_labels_test = data_normalized_test_onemonth_modification['label'].values[time_steps - 1:time_steps - 1 + len(predicted_modified_month)]

print("\n[IF based on MSE and MAE agreement (both must be anomaly)]")
print(f"Precision: {precision_score(true_labels_test, final_anomaly_labels):.4f}")
print(f"Recall:    {recall_score(true_labels_test, final_anomaly_labels):.4f}")
print(f"F1 Score:  {f1_score(true_labels_test, final_anomaly_labels):.4f}")
print(f"Accuracy:  {accuracy_score(true_labels_test, final_anomaly_labels):.4f}")

cm = confusion_matrix(true_labels_test, final_anomaly_labels)
labels = ['Normal', 'Anomaly']
